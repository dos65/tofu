...
object LoggingMidFunctions {
  ...
  def errIn[U[_[_]]: FunctorK, I[_]: Functor, F[+_]: Monad, E](implicit
      ...
      errs: Errors[F, E],
  ): I[U[Mid[F, _]]] = logs.forService[U[F]].map(implicit logging => lmid.mapK(funK[LoggingErrMid[E, _], Mid[F, _]](_.toMid[F])))

  def namedErr[U[_[_]]: FunctorK, I[_]: Functor, F[+_]: Monad, E](name: String)(implicit
      logs: Logs[I, F],
      lmid: LoggingErrMid.Of[U, E],
      errs: Errors[F, E],
  ): I[U[Mid[F, _]]] = logs.byName(name).map(implicit logging => lmid.mapK(funK[LoggingErrMid[E, _], Mid[F, _]](_.toMid[F])))
}
