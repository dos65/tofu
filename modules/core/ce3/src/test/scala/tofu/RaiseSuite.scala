...
object RaiseSuite {
  ...
  def foo0[F[
      _
  ]: ({ type L[x[_]] = Raise[x[_], ConcreteError] })#L: ({ type L[x[_]] = Raise[x[_], AnotherConcreteError] })#L]
      : F[Unit] = {
  ...
  @unused212
  def foo4[
    F[_]: ({ type L[x[_]] = ContravariantRaise[x[_], ConcreteError] })#L: ({ type L[x[_]] = ContravariantRaise[x[_], AnotherConcreteError] })#L
  ]: F[Unit] =
    ConcreteError().raise[F, Unit]
